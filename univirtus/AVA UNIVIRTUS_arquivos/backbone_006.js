...
(function (factory) {
    ...
    _.extend(Backbone.Router.prototype, {
        ...
        _setTitle: function (routeName) {
            var title = this.titles[routeName], newTitle, makeTitle;
            ...
            makeTitle = function makeTitle (ctx) {
                var titles = [ctx.titlePrefix, newTitle, ctx.titleSuffix];
                return _.without(titles, '').join(ctx.titleSeparator);
            };

            document.title =  makeTitle(this);
        ...
        route: function (route, name, callback) {
            var wrappedCallback = function () {
                ...
                var makeName = function makeName (name, args) {
                    if ( args[0] === 'ava' ) {
                        name = (args[3]) ? args[1] + '_' + args[3]: args[1];
                    }
                ...
                if (this.titles) {
                    this._setTitle(makeName(name, arguments));
                }
            };
